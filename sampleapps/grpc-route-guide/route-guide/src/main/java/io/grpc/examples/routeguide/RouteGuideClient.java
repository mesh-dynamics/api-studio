...
//    }
  ...
  public void listFeaturesWithNote(int lowLat, int lowLon, int hiLat, int hiLon) {
    info("*** ListFeatures: lowLat={0} lowLon={1} hiLat={2} hiLon={3}", lowLat, lowLon, hiLat,
        hiLon);
    ...
    try {
      featuresWithNotes = blockingStub.listFeaturesWithNote(request);
  ...
  public void recordRouteWithNote(List<Feature> features, int numPoints) throws InterruptedException {
    info("*** RecordRoute");
    final CountDownLatch finishLatch = new CountDownLatch(1);
    StreamObserver<RouteSummaryWithNote> responseObserver = new StreamObserver<RouteSummaryWithNote>() {
      @Override
    ...
    StreamObserver<Point> requestObserver = asyncStub.recordRouteWithNote(responseObserver);
    try {
      // Send numPoints points randomly selected from the features list.
      for (int i = 0; i < numPoints; ++i) {
        int index = random.nextInt(features.size());
  ...
  public CountDownLatch routeChatWithNote() {
    info("*** RouteChat");
    final CountDownLatch finishLatch = new CountDownLatch(1);
    StreamObserver<RouteNote> requestObserver =
        asyncStub.routeChatWithNote(new StreamObserver<RouteNoteWithNote>() {
          @Override
...
//    String target = "localhost:8980";
    String target = "grpc.dev.meshdynamics.io:8980";
...
//       Looking for features between 40, -75 and 42, -73.
        ...
        client.listFeaturesWithNote(400000000, -750000000, 420000000, -730000000);

//       Record a few randomly selected points from the features file.
        client.recordRoute(features, 10);
        client.recordRouteWithNote(features, 10);

//       Send and receive some notes.
        CountDownLatch finishLatch1 = client.routeChat();
        if (!finishLatch1.await(1, TimeUnit.MINUTES)) {
          client.warning("routeChat can not finish within 1 minutes");
        }
//
        CountDownLatch finishLatch = client.routeChatWithNote();
        if (!finishLatch.await(1, TimeUnit.MINUTES)) {
          client.warning("routeChat can not finish within 1 minutes");
        }
        ...
        client.recordRouteWithNote(features, 5);

            CountDownLatch finishLatch2 = client.routeChat();
        if (!finishLatch2.await(1, TimeUnit.MINUTES)) {
          client.warning("routeChat can not finish within 1 minutes");
        }
//
        CountDownLatch finishLatch3 = client.routeChatWithNote();
        if (!finishLatch3.await(1, TimeUnit.MINUTES)) {
          client.warning("routeChat can not finish within 1 minutes");
        }
