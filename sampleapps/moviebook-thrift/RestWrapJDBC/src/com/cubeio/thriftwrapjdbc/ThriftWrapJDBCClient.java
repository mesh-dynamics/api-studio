...
public class ThriftWrapJDBCClient {
	...
	public static void main(String[] args) {
		...
		try (Scope scope = startClientSpan("ThriftQuery");) {
			...
			String queryString = "select film.film_id as film_id, film.title as title, group_concat("
                + "actor_film_count.first_name) as actors_firstnames, group_concat(actor_film_count.last_name)"
                + " as actors_lastnames, group_concat(actor_film_count.film_count) as film_counts from film,"
                + " film_actor, actor_film_count  where film.film_id = film_actor.film_id and "
                + "film_actor.actor_id = actor_film_count.actor_id  and title = ? group by film.film_id, film.title";
