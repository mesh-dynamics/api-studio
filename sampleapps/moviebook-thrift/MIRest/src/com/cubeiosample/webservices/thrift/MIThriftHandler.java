...
public class MIThriftHandler implements MIThrift.Iface {

	private static Logger LOGGER = Logger.getLogger(MIThriftHandler.class);
	...
	static {
		LOGGER = Logger.getLogger(MIThriftHandler.class);
	...
	public ListMovieResult listMovies(String filmName, String keyWord, String actor, Span span)
		...
		try (Scope scope = CommonUtils.startServerSpan(span, "listMovies")) {
			LOGGER.info(
				"Current span In List Movies (Started From Null):: " + scope.span().toString());
			films = lmc.getMovieList(filmName);
			if (films != null) {
				films.forEach(jsonObj -> listMovieResult
					.addToMovieInfoList(convertJsonToObject((JSONObject) jsonObj)));
			}
	...
	@Override
	public RentMovieResult rentMovie(RentalInfo rentalInfo, Span span)
		throws GenericMIRestException, TException {
	...
	@Override
	public ListStoreResult listStores(int filmId, Span span)
		throws GenericMIRestException, TException {
