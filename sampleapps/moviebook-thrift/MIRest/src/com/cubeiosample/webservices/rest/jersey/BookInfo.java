...
public class BookInfo {
	...
	public BookInfo(Tracer tracer, Config config) {
		...
		randomGuassianPercentGivenStdDevAndMean =
			random.nextGaussian() * config.FAIL_PERCENT_STD_DEV + config.FAIL_PERCENT;
	...
	public JSONObject getBookInfo(String title, int id) {
		...
		try {
            ...
            /*
                ...
                unlike for the other two ('ratings' and 'reviews' apps, where the code to fail
                the response randomly is written).
                Ideally we can do in the details (ruby application) but since not familiar
                with the ruby syntax at this point of time, doing it here temporarily.
             */
           ...
           /* if (random.nextDouble() < randomGuassianPercentGivenStdDevAndMean) {
			...
			LOGGER.error(
				String.format("getBookInfo failed: %s, params: %d; %s", title, id, e.toString()));
			if (response != null) {
