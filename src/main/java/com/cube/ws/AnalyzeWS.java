...
//        Collection<MatchResultAggregate> res = rrstore.computeResultAggregate(replayId, service, bypath);
    ...
    public Response getAnalysisResultWithoutTrace(@Context UriInfo urlInfo, @PathParam("recordReqId") String recordReqId,
        ...
        return matchResult.map(matchRes -> {
            Optional<Request> request = rrstore.getRequestEvent(recordReqId).flatMap(event -> Request.fromEvent(event, jsonMapper));
            Optional<com.cube.dao.Response> recordedResponse = rrstore.getResponseEvent(recordReqId)
            ...
            Optional<com.cube.dao.Response> replayedResponse = matchRes.replayReqId.flatMap(rrstore::getResponseEvent)
                .flatMap(event -> com.cube.dao.Response.fromEvent(event, jsonMapper));
...
//            Collection<MatchResultAggregate> res = rrstore.computeResultAggregate(replayId, service, bypath);
    ...
    public Response getResultByReq(@Context UriInfo uriInfo,
        ...
        Optional<com.cube.dao.Response> recordResponse = recordReqId.flatMap(rrstore::getResponseEvent)
            .flatMap(event -> com.cube.dao.Response.fromEvent(event, jsonMapper));
        Optional<com.cube.dao.Response> replayResponse = replayReqId.flatMap(rrstore::getResponseEvent)
            .flatMap(event -> com.cube.dao.Response.fromEvent(event, jsonMapper));
    ...
    public Response updateGoldenSet(@PathParam("recordingId") String recordingId,
        ...
        try {
            ...
            Recording updatedRecording = new Recording(originalRec.customerId,
                ...
                Optional.of(collectionUpdateOpSetId), Optional.of(templateUpdOpSetId), comment, userId, Optional.empty());
    ...
    public Response sanitizeRecording (@QueryParam("recordingId") String recordingId,
        ...
        try {
            ...
            Recording updatedRecording = new Recording(originalRec.customerId,
                originalRec.app, originalRec.instanceId, newCollectionName,
                Recording.RecordingStatus.Completed,
                Optional.of(Instant.now()), templateSet.version, Optional.of(originalRec.getId()),
                Optional.of(originalRec.rootRecordingId), originalRec.name, originalRec.codeVersion,
                originalRec.branch,
                originalRec.tags, originalRec.archived, originalRec.gitCommitId, Optional.empty(),
                Optional.empty(), Optional.empty(), originalRec.userId,
                originalRec.generatedClassJarPath);
