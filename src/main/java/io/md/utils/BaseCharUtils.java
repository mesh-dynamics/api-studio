...
class BaseCharUtils {
	static final String alphabetsSmallLetters = IntStream
		.rangeClosed((int)'a' , (int)'z' ).mapToObj(c->Character.valueOf((char) c).toString()).collect(
			Collectors.joining());
	...
	public static String padding(int length){
		...
		if(padding==null) {
			...
			for (int i = 0 ; i < length ; i++) {
				padding = padding.concat(PADDING_CHAR);
			}
