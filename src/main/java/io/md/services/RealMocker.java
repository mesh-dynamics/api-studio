...
public class RealMocker implements Mocker {
    ...
    public Optional<Event> getResp(MultivaluedMap<String, String> queryParams, String path,
        ...
        LOGGER.info(Utils.createLogMessasge(Constants.MESSAGE, "Attempting to mock request",
        ...
        Optional<RecordOrReplay> recordOrReplay = cube
        ...
        Optional<String> replayIdOpt = recordOrReplay
            .flatMap(RecordOrReplay::getCollection);
        ...
        if (!replayIdOpt.isPresent() || !collectionOpt.isPresent()) {
            LOGGER.error(Utils.createLogMessasge(
                Constants.MESSAGE, "Unable to mock request since replay/collection is empty",
                Constants.CUSTOMER_ID_FIELD, customerId, Constants.APP_FIELD, app
                , Constants.INSTANCE_ID_FIELD, instanceId, Constants.SERVICE_FIELD, service,
        ...
        try {
            ...
            LOGGER.error(Utils.createLogMessasge(
                Constants.MESSAGE, "Unable to mock request since request comparator not found",
                    Constants.CUSTOMER_ID_FIELD, customerId, Constants.APP_FIELD, app
                    , Constants.INSTANCE_ID_FIELD, instanceId, Constants.SERVICE_FIELD, service,
                    Constants.METHOD_FIELD, method, Constants.PATH_FIELD, path, Constants.BODY,
        ...
        try {
            ...
            LOGGER.error(Utils.createLogMessasge(
                Constants.MESSAGE, "Unable to mock request, exception while creating request",
                    Constants.CUSTOMER_ID_FIELD, customerId, Constants.APP_FIELD, app
                    , Constants.INSTANCE_ID_FIELD, instanceId, Constants.SERVICE_FIELD, service,
                    Constants.METHOD_FIELD, method, Constants.PATH_FIELD, path, Constants.BODY,
                    body, Constants.REPLAY_ID_FIELD , replayId), e);
        ...
        Optional<Event> respEventGolden = cube.getSingleEvent(reqQuery)
            .flatMap(event -> cube.getRespEventForReqEvent(event));
        if (respEventGolden.isPresent()) {
            respEvent = respEventGolden;
        } else {
            LOGGER.info(Utils.createLogMessasge(
                Constants.MESSAGE, "Using default response(as no matching request event found)",
                Constants.CUSTOMER_ID_FIELD, customerId, Constants.APP_FIELD, app
                , Constants.INSTANCE_ID_FIELD, instanceId, Constants.SERVICE_FIELD, service,
                Constants.METHOD_FIELD, method, Constants.PATH_FIELD, path,
                Constants.TRACE_ID_FIELD,
                String.join(":", reqQuery.getTraceIds()), Constants.REPLAY_ID_FIELD,
                replayId));
            ...
            if (defRespEvent.isPresent()) {
                respEvent = defRespEvent;
            } else {
                LOGGER.error(Utils.createLogMessasge(
                    Constants.MESSAGE,
                    "Unable to mock request since no default response found",
                    Constants.CUSTOMER_ID_FIELD, customerId, Constants.APP_FIELD, app
                    , Constants.INSTANCE_ID_FIELD, instanceId, Constants.SERVICE_FIELD,
                    service,
                    Constants.METHOD_FIELD, method, Constants.PATH_FIELD, path,
                    Constants.BODY,
        ...
        if (respEvent.isPresent()) {
            createResponseFromEvent(mockRequestEvent, respEvent.get());
        } else {
            ...
            ReqRespMatchResult matchResult =
                new ReqRespMatchResult(Optional.empty()
                    , Optional.ofNullable(mockRequestEvent.reqId),
                    Comparator.MatchType.NoMatch
                    ...
                    new Comparator.Match(
                        Comparator.MatchType
                            .Default, "", Collections.emptyList()),
                    new Comparator.Match(Comparator.MatchType
                        .Default, "", Collections.emptyList()));
    ...
    private static EventQuery buildFunctionEventQuery(Event event, int offset, int limit, boolean isSortOrderAsc) {
        return new EventQuery.Builder(event.customerId, event.app, Event.EventType.JavaRequest)
            ...
            .withPaths(Arrays.asList(event.apiPath)).withTraceId(event.getTraceId()).withTimestamp(event.timestamp)
    ...
    private FnResponse getFuncResp(Event event, long matchingEventsCount, Event retEvent) throws MockerException {
        LOGGER.debug(Utils.createLogMessasge(
    ...
    private FnResponse getDefaultFuncResp(Event event) throws MockerException {
        ...
        defEventQuery.withPaths(Arrays.asList(event.apiPath));
        ...
        if (defaultRespEvent.isPresent()) {
            try {
                ...
                LOGGER.error(Utils.createLogMessasge(
                    Constants.API_PATH_FIELD, event.apiPath), e);
