...
def init_files(base_dir, template_dir, namespace, cube_application, cube_customer):
    ...
    solr_url=sys.argv[18]
    logcollector_url=sys.argv[19]
...
def recap_files(operation, base_dir, template_dir, namespace, cube_application, cube_customer):
    ...
    with open(outfile, "w") as out:
        for service in dataMap['services']:
            output_from_template = template.render(service_name=service, customer=cube_customer, cube_application=cube_application, cube_instanceid=cube_instanceid, cube_host=cube_host, namespace=namespace, auth_token=auth_token)
            out.write("---\n")
            out.write(output_from_template + "\n" * 2)
    out.close()
    if operation == "replay":
        ...
        if os.path.isfile(mock_file):
            ...
            with open(outfile, "w") as out:
                output_from_template = template.render(namespace=namespace, customer=cube_customer, cube_application=cube_application, cube_instance=cube_instanceid, cube_host=cube_host, auth_token=auth_token)
                out.write(output_from_template)
                out.close()
