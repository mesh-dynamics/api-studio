...
public class FindAndRentMovies {
	...
	private static String[] movies = {"ANACONDA CONFESSIONS", "AUTUMN CROW", "BEVERLY OUTLAW",
	...
	//  Generating query: select language_id, group_concat(concat('"', substring_index(title, ' ', 1),
	//  '"') separator ', ') from film where film_id % 29 = 0 group by language_id
	private static String[] movie_keywords = {"ANTITRUST", "BEACH", "BOONDOCK", "CANDIDATE",
		"CHISUM", "CONFIDENTIAL", "DAISY", "DIRTY", "DUFFEL", "EVERYONE", "FISH", "GANDHI",
		"GREASE", "HAUNTING", "HOTEL", "INTENTIONS", "KANE", "LIFE", "MAIDEN", "MINE", "MUSCLE",
		"OPERATION", "PEACH", "PRIDE", "REQUIEM", "RUSHMORE", "SHANE", "SMOKING", "STAR", "SWARM",
		"TOWERS", "UPTOWN", "WAR", "WONKA"};
	...
	private void warmMovieCache() {
		for (String movie : movies) {
			...
			if (rand % 10 == 0) {
				Response response = callWithRetries(
					targetService.path("listmovies").queryParam("filmName", movie)
						.request(MediaType.APPLICATION_JSON), null, true, 1);
				response.close();
			}
		}
	}
	...
	public void getToken() throws Exception {
		...
		Response response = callWithRetries(
			targetService.path("authenticate").request(MediaType.APPLICATION_JSON),
	...
	public void driveTraffic(Optional<Integer> numMovies) throws Exception {
        ...
        */
		...
		for (int i = 0; i < nm; i++) {
			...
			// list films
			Response response1 = callWithRetries(
				targetService.path("listmovies").queryParam("filmName", movie)
			...
			Response response2 = callWithRetries(
				targetService.path("liststores").queryParam("filmId", movieId)
					.request(MediaType.APPLICATION_JSON).header(HttpHeaders.AUTHORIZATION, token),
				null, true, 1);
			...
			Response response3 = callWithRetries(
				targetService.path("rentmovie").request().header(HttpHeaders.AUTHORIZATION, token),
				Entity.entity(rentalInfo.toString(), MediaType.APPLICATION_JSON), false,
				1); // TOFIX: why is it retrying? is it timing out while debugging?
			...
			Response response4 = callWithRetries(targetService.path("returnmovie").request()
	...
	private Response callWithRetries(Builder req, Entity<Object> body, boolean isGetRequest,
		int numRetries) {
		int numAttempts = 0;
		headers.forEach((k, v) -> {
			req.header(k, v);
