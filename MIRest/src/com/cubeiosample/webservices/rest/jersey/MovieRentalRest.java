...
// TODO: @Secured decorators are all commented out since we have to modify request matching
//  during replay and analysis to ignore certain fields.
...
public class MovieRentalRest {
	...
	public Response health() {
		return Response.ok().type(MediaType.APPLICATION_JSON)
			.entity("{\"MIRest status\": \"MovieInfo is healthy\"}").build();
	}
...
//							               @HeaderParam("x-ot-span-context") String xotspan) {
	...
	public Response findStoreswithFilm(@QueryParam("filmId") Integer filmId,
		...
		// select * from inventory, store, address where inventory.store_id = store.store_id and
		// store.address_id = address.address_id and (postal_code is not null and length(postal_code) > 3)
