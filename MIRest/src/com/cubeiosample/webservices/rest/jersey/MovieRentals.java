...
public class MovieRentals {
	...
	public JSONArray listMovieByName(String filmname) {
		...
		String query =
			"select film.film_id as film_id, film.title as title, group_concat(actor_film_count.first_name) "
				+ "as actors_firstnames, group_concat(actor_film_count.last_name) as actors_lastnames, "
				+ "group_concat(actor_film_count.film_count) as film_counts "
	...
	private String arrayifyOrRemoveRandomly(JSONObject jsonObject, String fieldName,
		Random forShuffle) {
	...
	private List<String> displayActors(String firstNames, String lastNames, String filmCounts) {
		LOGGER.debug(String
			.format("finding display actors for %s, %s, %s", firstNames, lastNames, filmCounts));
		...
		for (int i = 0; i < impActorIndexes.size(); ++i) {
			if (displayNameLastFirst) {
				result.add(buggyAppend(lNamesArr[impActorIndexes.get(i)],
					fNamesArr[impActorIndexes.get(i)], ",")/* + "; "*/);
			} else {
				result.add(buggyAppend(fNamesArr[impActorIndexes.get(i)],
					lNamesArr[impActorIndexes.get(i)], " ") /*+ "; "*/);
			}
		}
		return result;
	}
	...
	private String buggyAppend(String name1, String name2, String separator) {
		if (config.CONCAT_BUG) {
			...
			if (name1.toLowerCase().startsWith("a") || name1.toLowerCase().startsWith("p") || name1
	...
	private List<Integer> maxKIndexes(int[] counts, int k) {
		...
		while (k > indexes.size() && indexesToChooseRandomly.size() > 0 && num_iterations < 1000) {
			...
			if (Math.random() > 0.5) {
				indexes.add(
					indexesToChooseRandomly.get(num_iterations % indexesToChooseRandomly.size()));
	...
	private void enhanceFilmsWithReviews(JSONArray films) {
		...
		for (int i = 0; i < films.length(); ++i) {
			JSONObject film = films.getJSONObject(i);
			JSONObject binfo = bookInfo
				.getBookInfo(film.getString("title"), film.getInt("film_id"));
	...
	public JSONObject rentMovie(int film_id, int store_id, int duration, int customer_id,
		int staff_id) throws SQLException, JSONException {
		// Update rental table
...
//    }
	...
	public JSONArray rentFilmsBulk(JSONArray filmArray, int storeId, int duration, int customerId,
		int staffId) throws JSONException, SQLException {
		...
		for (Map.Entry<String, Integer> entry : films.entrySet()) {
			JSONObject obj = this
				.rentMovie(entry.getValue(), storeId, duration, customerId, staffId);
			rentals.put(obj);
		}
	...
	private JSONObject updateRental(int inventory_id, int customer_id, int staff_id)
		throws SQLException, JSONException {
		String dateString = format.format(new Date());
		String rentalUpdateQuery =
	...
	private int getRentalIdForReturn(int inventoryId, int customerId, int staffId) {
		try {
			String rentalIdForReturnQuery = "SELECT rental_id from rental WHERE inventory_id = ? "
				+ "and customer_id = ? and staff_id = ? and return_date is null";
			JSONArray params = new JSONArray();
			...
			LOGGER.info(
				"Couldn't find rental_id for [" + inventoryId + ", " + customerId + ", " + staffId
					+ "]");
			return -1;
		}
	}
	...
	private JSONObject returnRental(int inventoryId, int customerId, int staffId, double amount) {
		...
		String paymentQuery = "INSERT INTO payment (customer_id, staff_id, rental_id, amount, "
			+ "payment_date) VALUES (?, ?, ?, ?, ?)";
