...
COPY --from=build target/cubews-replay-V1-SNAPSHOT.war /usr/local/tomcat/webapps/ROOT.war
...
RUN rm -rf /usr/local/tomcat/webapps/ROOT
RUN perl -0777 -i -pe 's/securerandom.source=file:\/dev\/random/securerandom.source=file:\/dev\/urandom/' /etc/java-11-openjdk/security/java.security
ADD target/cubews-replay-V1-SNAPSHOT.war /usr/local/tomcat/webapps/ROOT.war
COPY server.xml /usr/local/tomcat/conf/server.xml
